...
var presionaPieza = function presionaPieza(fila, columna) {
    ...
    if (matrizDesorganiza[fila][columna].ocupado) {
        var replaceSpace = _utils2.default.accesoDOM(fila + "_" + columna);
            ...
            if (enPosicion) {
                ...
                if (matrizDesorganiza[posBusca.fila][posBusca.columna].ocupado === false) {
                    ...
                    for (var _i = 1; _i <= 2; _i++) {
                        if (_i == 1) {
                            var blackSpace = _utils2.default.accesoDOM([posBusca.fila] + "_" + [posBusca.columna]);
                            ...
                            var _style = _estiloCelda3.style;
                            var _clase = _estiloCelda3.clase;

                            replaceSpace.innerText = ".";
                            replaceSpace.classList.remove("fondo");
                            replaceSpace.setAttribute("style", _style);
                        }
                    }
                }
            }
    ...
    _utils2.default.accesoDOM("clicks").innerHTML = "<h3><font  color='blue'>Cantidad de clicks: " + click + "</font></h3>";
};
...
var desorganizaPuzzle = function desorganizaPuzzle() {
    ...
    for (var _i2 = 0; _i2 < matrizPuzzle.length; _i2++) {
        matrizDesorganiza.push([]);
        for (var c = 0; c < matrizPuzzle.length; c++) {
            var posicion = desorganiza[cont];
            matrizDesorganiza[_i2][c] = JSON.parse(JSON.stringify(matrizPuzzle[posicion.fila][posicion.columna]));
            cont++;
        }
    }
...
_utils2.default.accesoDOM("comprueba").addEventListener('click', function (event) {
    ...
    cont === matrizPuzzle.length ? _utils2.default.accesoDOM("message").innerHTML = "<h3><font  color='green'>Correcto</font></h3>" : _utils2.default.accesoDOM("message").innerHTML = "<h3><font  color='red'>Sigue intentando, validas " + cont + " </font></h3>";
});
